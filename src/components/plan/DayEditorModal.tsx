...
export function DayEditorModal({
  ...
  const save = async () => {
    const body = { name: name.trim() || WD[weekday], weekday: Number(weekday), focus: focus.trim() || null, isRest };
    if (day) await updateDayAction(day.id, body);
    else await createDayAction(cycleId, body);
    onSaved();
  };
  const del = async () => {
    if (!day) return;

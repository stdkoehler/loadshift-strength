...
const phases: Phase[] = [
  { id: 1, cycleId: 1, name: 'Hypertrophy', startWeek: 1, endWeek: 3, color: '#34d399', orderIndex: 1 },
  { id: 2, cycleId: 1, name: 'Strength', startWeek: 4, endWeek: 5, color: '#f87171', orderIndex: 2 },
];
...
describe('deriveState + buildPayload roundtrip', () => {
  it('non-phased uniform (constant): detects uniform structure and rebuilds identical targets', () => {
    const fixture = ex({
      progressionType: 'constant',
      sets: [
  ...
  it('non-phased custom (ramp): detects custom structure and preserves per-set reps/weight/role/rir', () => {
    const fixture = ex({
      ...
      sets: [
        ...
        { id: 2, exerciseId: 1, setIndex: 2, role: 'Top-Set', targets: [{ id: 2, setId: 2, phaseId: null, reps: 6, baseWeight: 149, incrementPerWeek: 0, targetRir: 1, incrementPerRepeat: 0 }] },
      ],
    });
    const state = deriveState(fixture, []);
    ...
    expect(payload.sets).toEqual([
      ...
      { role: 'Top-Set', targets: [{ phaseId: null, reps: 6, baseWeight: 149, incrementPerWeek: 0, targetRir: 1 }] },
    ]);
  });

  it('phased uniform: same reps/weight per phase across all sets, no roles, shared repeat-increment', () => {
    const fixture = ex({
      progressionType: 'phased',
      sets: [
        {
  ...
  it('phased custom (ramp): different reps/weight/rir per set AND per phase, roles shared across phases', () => {
    const fixture = ex({
      ...
      sets: [
        ...
        {
          id: 2, exerciseId: 1, setIndex: 2, role: 'Top-Set',
          targets: [
    ...
    expect(payload.sets).toEqual([
      ...
      { role: 'Top-Set', targets: [{ phaseId: 1, reps: 12, baseWeight: 5, incrementPerWeek: 0, targetRir: 2, incrementPerRepeat: 1.25 }, { phaseId: 2, reps: 5, baseWeight: 30, incrementPerWeek: 0, targetRir: 0, incrementPerRepeat: 1.25 }] },
    ]);
  });

...
export function RampPhased({
  ...
  const removeRow = (i: number) => patch({ customPhase: st.customPhase.filter((_, j) => j !== i) });
  const addRow = () => {
    ...
    patch({ customPhase: [...st.customPhase, { role: '', vals }] });
  };
        ...
        return (
          <div key={i} className="flex flex-col gap-2 rounded-md border border-neutral-800 p-2">
            <div className="grid grid-cols-[1fr_1fr_1fr_1fr_auto] items-center gap-2">
              ...
              <input inputMode="numeric" placeholder="Wdh" className={inputClass} value={v.reps} onChange={(e) => updateVal(i, 'reps', e.target.value)} />
              <input inputMode="decimal" placeholder="kg" className={inputClass} value={v.weight} onChange={(e) => updateVal(i, 'weight', e.target.value)} />
              <input inputMode="numeric" placeholder="–" className={inputClass} value={v.rir ?? ''} onChange={(e) => updateVal(i, 'rir', e.target.value)} />
              <button type="button" onClick={() => removeRow(i)} className="px-2 text-neutral-500 hover:text-red-400" aria-label="Satz entfernen">
                ✕
              </button>
            </div>
            <select className={inputClass} value={c.role || ''} onChange={(e) => updateRole(i, e.target.value)}>
